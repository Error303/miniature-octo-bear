...
<body>
	...
	foreach ($synaitems as $key => $value) {
		...
		$matches = array();
		$levenmatches = array();
		$regex = '(' . implode('|', $hyvat_synat) . ')';
		$num_found = preg_match_all($regex, $string, $matches);

		if ($num_found == 0) {
			foreach ($hyvat_synat as $hyva_syna) {
				if (levenshtein(strtolower($string), strtolower($hyva_syna)) <= $levenmatchfactor) {
					$levenmatches[] = $hyva_syna;
				}
			}
			if (isset($levenmatches[0])) {
				print 'Oiskohan tää jees? Muistuttaa vähäsen: ' . implode(', ', $levenmatches) . '<br/><a href="'.$value['link']['value'].'">'. $string .'</a><br/>';
			}			
			
		} else {
			print 'Jee, <a href="'.$value['link']['value'].'">'. $string .'</a> ois myynnis!<br/><br/>';
		}
	}

	foreach ($sambleritems as $key => $value) {
		$string = strstrbi($value['title']['value'], 'Myydään: ', FALSE, FALSE);
		$string = strstrbi($string, ' (', TRUE, FALSE);
		$matches = array();
		$levenmatches = array();
